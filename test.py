...
def get_cross_kernel(corner_len=15, thick=1):
    c_len = corner_len

    cross_kernel = np.zeros((2*c_len+thick, 2*c_len+thick))
    for i in range(c_len+thick):
        div = thick*(2*c_len+thick)
        for t in range(-thick//2, thick//2):
            cross_kernel[t][t] = 1/div
...
def find_corner_by_corner_canny(corner, canny):
    ...
    # corner = cv2.filter2D(corner, -1, get_cross_kernel())
    # corner = cv2.filter2D(corner, -1, np.array([[1/4, 1/4], [1/4, 1/4]]))
    # cv2.imshow("corner", cv2.equalizeHist((corner*255).astype(np.uint8)))
    # cv2.waitKey()
    gray_sort = np.sort(corner.flatten())[::-1]
    coordinate = np.where(corner > gray_sort[100])
    coordinate = coordinate[::-1]
    coordinate = list(map(list, zip(*coordinate)))
    return coordinate  # (x, y)


def get_possible_rectangle(lu_coord, ru_coord, rd_coord, ld_coord, img):
    blur_thr = 2
...
if __name__ == '__main__':
    ...
    # img = cv2.imread('D:/github_project/auto_press_down_gun/image_detect/screen_captures/weapon/98k/98k.png')2188
    ...
    possible_rect = get_possible_rectangle(lu_coor, ru_coor, rd_coor, ld_coor, img)

    cp2_img = img.copy()
    for x, y, x1, y1 in possible_rect:
        cv2.circle(cp2_img, (x, y), 5, (255, 0, 0), -1)
    for x, y, x1, y1 in possible_rect:
        cv2.circle(cp2_img, (x1, y), 2, (0, 255, 0), -1)
    for x, y, x1, y1 in possible_rect:
        cv2.circle(cp2_img, (x1, y1), 2, (0, 0, 255), -1)
    for x, y, x1, y1 in possible_rect:
        cv2.circle(cp2_img, (x, y1), 2, (0, 255, 255), -1)
